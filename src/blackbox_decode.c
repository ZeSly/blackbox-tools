...
void fprintfGPSFields(FILE *file, uint64_t fieldvalue, int i, bool unitName)
{
    ...
    case GPS_FIELD_TYPE_COORDINATE_DEGREES_TIMES_10000000:
        degrees = fieldvalue / 10000000;
        fracDegrees = llabs(fieldvalue) % 10000000;
...
void fprintfSlowFrameFields(flightLog_t *log, FILE* file,int64_t fieldValue, int i)
{
    ...
    if ((i == log->slowFieldIndexes.flightModeFlags || i == log->slowFieldIndexes.stateFlags)
        ...
        if (i == log->slowFieldIndexes.flightModeFlags) {
            flightlogFlightModeToString(log, fieldValue, buffer, BUFFER_LEN);
        }
        else {
            flightlogFlightStateToString(log, fieldValue, buffer, BUFFER_LEN);
        }
...
void outputSlowFrameFields(flightLog_t *log, int64_t *frame)
{
    ...
    for (int i = 0; i < log->frameDefs['S'].fieldCount; i++) {
        ...
        fprintfSlowFrameFields(log, csvFile, frame[i], i);
    }
}
...
void outputMainFrameFields(flightLog_t *log, int64_t frameTime, int64_t *frame)
{
    ...
    for (i = 0; i < log->frameDefs['I'].fieldCount; i++) {
        ...
        if (i == FLIGHT_LOG_FIELD_INDEX_TIME) {
            ...
            if (frameTime == -1) {
                fprintf(csvFile, "X");
            } else if (!fprintfMainFieldInUnit(log, csvFile, i, frameTime, false)) {
                fprintf(stderr, "Bad unit for field %d\n", i);
                exit(-1);
            }
        } else if (!fprintfMainFieldInUnit(log, csvFile, i, frame[i], false)) {
            fprintf(stderr, "Bad unit for field %d\n", i);
            exit(-1);
        }
    }
...
void applyFieldUnits(flightLog_t *log)
{
    if (options.raw) {
        ...
        if (log->mainFieldIndexes.rssi > -1) {
            mainFieldUnit[log->mainFieldIndexes.rssi] = options.unitRssi;
        }
...
int decodeFlightLog(flightLog_t *log, const char *filename, int logIndex)
{
    ...
    if (options.toStdout) {
        ...
        filenameLen = outputPrefixLen + strlen(".00.gpx") + 1;
        gpxFilename = malloc(filenameLen * sizeof(char));

        snprintf(gpxFilename, filenameLen, "%.*s.%02d.gpx", outputPrefixLen, outputPrefix, logIndex + 1);
...
{
    fprintf(stderr,
        "Blackbox flight log decoder by Sylvain Girard, based on the work of Nicholas Sherlock ("
